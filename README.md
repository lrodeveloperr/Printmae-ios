...
`App/` is the Japanese-first SwiftUI host with an optional English display
setting for review. Generate `PrintMaeApp.xcodeproj` with
...
The native host is connected to the local engine package for document import,
preflight, repair, preview rendering, verified export, purchase, sharing and
history restoration. The UI has six main task screens (prepare, report, fix plan, preview, print
...
See [the integration guide](Engine/Docs/INTEGRATION.md) and [release gates](Engine/Docs/RELEASE_GATE.md) for native build and device verification.
